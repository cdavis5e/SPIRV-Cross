...
float2 lerp_vertex(thread const float2& tess_coord, float2 vOutPatchPosBase, constant UBO& v_31)
{
    return vOutPatchPosBase + (tess_coord * v_31.uPatchSize);
}

float2 lod_factor(thread const float2& tess_coord, float4 vPatchLods)
{
