...
void CompilerMSL::build_implicit_builtins()
{
	...
	if (needs_aux_buffer_def || need_vertex_params || need_tesc_params)
	{
	...
	case StorageClassInput:
		...
		if (get_execution_model() == ExecutionModelTessellationControl)
		{
			...
			entry_func.fixup_hooks_in.push_back([=]() {
				...
				statement("if (", to_expression(builtin_invocation_id_id), " < ", to_expression(aux_buffer_id), ".",
	...
	for (auto p_var : vars)
	{
		bool strip_array =
		    (get_execution_model() == ExecutionModelTessellationControl ||
		     (get_execution_model() == ExecutionModelTessellationEvaluation && storage == StorageClassInput)) && !patch;
	...
	case OpPtrAccessChain:
		if (is_tessellation_shader())
		{
			...
			if (get_execution_model() == ExecutionModelTessellationControl && var && m &&
			    m->decoration.builtin_type == BuiltInTessLevelInner &&
